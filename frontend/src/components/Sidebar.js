        ...
        <div class="character-group">
            <div onclick="window.personaApp.toggleCharacterExpansion(${char.id})" class="character-header group p-3 md:p-4 rounded-xl cursor-pointer transition-all duration-200 relative hover:bg-gray-800/50">
                <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex space-x-1 z-10">
                    <button onclick="window.personaApp.createNewChatRoomForCharacter(${char.id}); event.stopPropagation();" class="p-1 bg-gray-700 hover:bg-blue-600 rounded text-gray-300 hover:text-white transition-colors" title="${t('sidebar.newChatRoom')}">
                        <i data-lucide="plus" class="w-3 h-3"></i>
                    </button>
                    <button data-id="${char.id}" class="edit-character-btn p-1 bg-gray-700 hover:bg-gray-600 rounded text-gray-300 hover:text-white transition-colors" title="${t('common.edit')}">
                        <i data-lucide="edit-3" class="w-3 h-3"></i>
                    </button>
                    <button data-id="${char.id}" class="delete-character-btn p-1 bg-gray-700 hover:bg-red-600 rounded text-gray-300 hover:text-white transition-colors" title="${t('common.delete')}">
                ...
                <div class="flex items-center space-x-3 md:space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-500 mt-1">${chatRooms.length}${t('sidebar.chatRoomCount')}</p>
                    </div>
                </div>
            </div>
            ${isExpanded ? `
...
function renderChatRoomItem(app, chatRoom) {
    ...
    const actionButtons = isEditing
        ? `<button data-chat-room-id="${chatRoom.id}" class="confirm-rename-btn p-1 bg-green-600 hover:bg-green-700 rounded text-white" title="${t('common.confirm')}">
               <i data-lucide="check" class="w-3 h-3"></i>
           </button>`
        : `<button data-chat-room-id="${chatRoom.id}" class="rename-chat-room-btn p-1 bg-gray-700 hover:bg-blue-600 rounded text-white" title="${t('sidebar.rename')}">
               <i data-lucide="edit-3" class="w-3 h-3"></i>
           </button>
           <button data-chat-room-id="${chatRoom.id}" class="delete-chat-room-btn p-1 bg-red-600 hover:bg-red-700 rounded text-white" title="${t('sidebar.deleteChatRoom')}">
               <i data-lucide="trash-2" class="w-3 h-3"></i>
           </button>`;
    ...
    sidebarContent.innerHTML = `
        <header class="p-4 md:p-6 border-b border-gray-800">
            ...
            <div class="relative">
                ...
                <input id="search-input" type="text" placeholder="${t('sidebar.searchPlaceholder')}" value="${app.state.searchQuery}" class="w-full pl-11 pr-4 py-2 md:py-3 bg-gray-800 text-white rounded-xl border-0 focus:ring-2 focus:ring-blue-500/30 focus:bg-gray-750 transition-all duration-200 text-sm placeholder-gray-500" />
            </div>
        </header>
