...
export class CustomOpenAIClient implements LLMApi {
	client: ChatOpenAI;
	constructor(
		apiKey: string,
		model: string,
	...
	async generateContent({
		...
		for (const msg of contents) {
			if (msg.role === "user") {
				messages.push(
					new HumanMessage("text" in msg.parts[0] ? msg.parts[0].text : ""),
				);
			} else if (msg.role === "assistant") {
				messages.push(
					new AIMessage("text" in msg.parts[0] ? msg.parts[0].text : ""),
				);
			}
		}
		...
		try {
			...
			throw new Error(t("api.invalidResponse", { provider: "Custom OpenAI" }));
		} catch (error) {
...
export default CustomOpenAIClient;
