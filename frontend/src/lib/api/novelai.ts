...
export class NovelAIClient {
	...
	validateModel(modelName: string): keyof typeof NOVELAI_MODELS | null {
		...
		const modelKey = (Object.keys(NOVELAI_MODELS) as ModelKey[]).find((key) => {
			const model: NovelAIModel | undefined = NOVELAI_MODELS[key];
			return (
				key
					.toLowerCase()
					.includes(modelName.toLowerCase()) || // Ignore case partial match
				model?.name.toLowerCase().includes(modelName.toLowerCase()) || // Name partial match
				model?.version === modelName // Version exact match
			);
		});

		if (modelKey) {
			console.warn(`[NAI] Model name autofix: "${modelName}" → "${modelKey}"`);
			return modelKey;
		}
	...
	async waitForNextGeneration(): Promise<void> {
		...
		const requiredDelay =
			this.options.minDelay + Math.floor(this.options.maxAdditionalDelay / 2);
	...
	async generateSticker(
		...
		const imageSize =
			mergedSettings.imageSize || mergedSettings.preferredSize || "square";
		const sizeConfig =
