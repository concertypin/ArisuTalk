    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center mb-1">
        ...
        <div class="flex items-center gap-1 shrink-0 ml-auto">
          <button
            on:click|stopPropagation={openSns}
            class="p-1.5 hover:bg-gray-700 rounded-full transition-colors z-20"
          ...
          {#if totalUnreadCount > 0}
            <span
              class="bg-red-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full leading-none"
      ...
      <div class="flex justify-between items-start">
        <p
          class="text-base line-clamp-2 {lastMessage?.isError
            ? 'text-red-400'
            : 'text-gray-400'} pr-4"
        >
          {#if lastMessage}
