<script>
  ...
  function openImageZoom() {
    if (message.type === 'image' || message.type === 'sticker') {
      const imageUrl = message.type === 'sticker' ? message.sticker.data : message.imageUrl;
      const title = message.type === 'sticker' ? message.sticker.stickerName : t('mainChat.uploadPhoto');
...
<div class="flex items-start gap-3" class:justify-end={message.isMe}>
  ...
  <div class="flex flex-col" class:items-end={message.isMe}>
    {#if showSenderInfo}
        <span class="text-sm text-gray-400 ml-2 mb-1">{message.sender.name}</span>
    {/if}
    <div 
    ...
    >
      {#if message.type === 'image'}
        <img src={message.imageUrl} alt="user upload" class="rounded-lg max-w-full h-auto cursor-pointer" on:click={openImageZoom}>
        {#if message.content}
          <p class="mt-2">{message.content}</p>
        {/if}
      {:else if message.type === 'sticker'}
        <div class="flex flex-col items-center cursor-pointer" on:click={openImageZoom}>
