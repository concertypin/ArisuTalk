...
{:else}
  <div class="inline-block px-4 py-2 rounded-2xl text-sm md:text-base leading-relaxed break-words {message.isError ? 'bg-red-500 text-white' : message.isMe ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200'}">
    <p>{message.content}</p>
  </div>
{/if}