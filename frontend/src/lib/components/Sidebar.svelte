...
<aside
  id="sidebar"
  class="fixed inset-y-0 left-0 z-30 flex w-full flex-col bg-gray-900 transition-transform duration-300 ease-in-out md:relative md:w-100 md:border-r md:border-gray-800 {$isSidebarCollapsed
    ? '-translate-x-full md:translate-x-0'
    : 'translate-x-0'}"
>
  <button
    on:click={toggleSidebar}
    class="absolute top-1/2 -translate-y-1/2 -right-4 h-8 w-8 items-center justify-center rounded-full border border-gray-600 bg-gray-700 transition-colors hover:bg-gray-600 md:flex z-20"
  >
      ...
      <div class="flex items-center justify-between mb-4 md:mb-6 gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="p-2 md:p-2.5 rounded-full bg-gray-800 hover:bg-gray-700 transition-all duration-200"
          >
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="p-4">
        <button
          ...
          class="w-full flex items-center justify-center py-3 md:py-3.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all duration-200 font-medium shadow-lg text-sm"
        >
      ...
      <div class="space-y-1 px-3 pb-4">
        ...
        <div class="border-t border-gray-800 pt-6">
          <div
            class="group flex items-center justify-between px-1 mb-2 relative"
          >
            <div class="flex items-center gap-2">
          ...
          {#each filteredCharacters as char (char.id)}
            ...
            >
              <div
                on:click={() => toggleCharacterExpansion(char.id)}
                on:keydown={(e) => {
                ...
                <div class="flex items-center space-x-4 md:space-x-5">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs md:text-sm truncate text-gray-400">
                      {lastAiMessage?.content?.substring(0, 35) ||
                        t("sidebar.startNewChat")}
                    </p>
                    <p class="text-xs text-gray-500 mt-1">
                      {rooms.length}{t("sidebar.chatRoomCount")}
                    </p>
                  </div>
                </div>
                <div
                  class="absolute bottom-3 right-3 flex space-x-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                >
                  <button
                    on:click|stopPropagation={() =>
                      createNewChatRoomForCharacter(char.id)}
                  ...
                  <button
                    on:click|stopPropagation={() => editCharacter(char)}
                    class="p-1.5 bg-gray-700/50 hover:bg-orange-500 rounded-lg text-gray-300 hover:text-white transition-colors"
                    title={t("common.edit")}
                  >
                    <Edit3 class="w-3.5 h-3.5" />
                  </button>
                  <button
                    on:click|stopPropagation={() => deleteCharacter(char)}
                    class="p-1.5 bg-gray-700/50 hover:bg-red-600 rounded-lg text-gray-300 hover:text-white transition-colors"
              ...
              {#if isExpanded}
                ...
                >
                  {#each rooms as room (room.id)}
                    {@const lastMessage =
                      $messages[room.id]?.[$messages[room.id]?.length - 1]}
                    {@const unreadCount = $unreadCounts[room.id] || 0}
                    <div class="chat-room-list-item-wrapper relative group">
                      <div
                        class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex space-x-1 z-10"
                      >
                        <button
                          on:click|stopPropagation={() => editChatRoom(room)}
                          class="p-1 bg-gray-700 hover:bg-gray-600 rounded text-gray-300 hover:text-white transition-colors"
                      ...
                      <div
                        on:click|stopPropagation={() => selectChat(room.id)}
                        ...
                        role="button"
                        tabindex="0"
                        class="chat-room-item p-3 rounded-lg cursor-pointer transition-all duration-200 w-full {$selectedChatId ===
                        room.id
                      ...
                      >
                        ...
                        {:else}
                          <div class="flex items-center justify-between">
                            ...
                            {#if unreadCount > 0}
                              <span
                                class="text-xs font-bold bg-red-500 text-white rounded-full px-1.5 py-0.5"
                                >{unreadCount}</span
                              >
                            {/if}
                          </div>
                          <div
                            class="flex items-center justify-between text-xs mt-1 {$selectedChatId ===
                            room.id
                              ? 'text-gray-200'
                              : 'text-gray-400'}"
                          >
                            <p class="truncate flex-1 pr-2">
                              {lastMessage?.content ||
