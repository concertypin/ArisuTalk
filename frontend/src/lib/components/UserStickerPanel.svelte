...
<div
  class="absolute bottom-full right-0 z-20 mb-2 w-80 rounded-xl shadow-lg border border-gray-700 animate-fadeIn floating-panel bg-gray-800 flex flex-col"
>
  <div
    class="p-3 border-b border-gray-700 flex items-center justify-between shrink-0"
  >
    <h3 class="text-sm font-medium text-white">
      {t("mainChat.personaStickers")}
    </h3>
    <div class="flex gap-2">
      <button
        on:click={addStickers}
        class="p-1 bg-blue-600 hover:bg-blue-700 text-white rounded"
        title="Add Sticker"
      >
        <Plus class="w-3 h-3" />
      </button>
      <button
        on:click={() => isUserStickerPanelVisible.set(false)}
        class="p-1 bg-gray-600 hover:bg-gray-500 text-white rounded"
        title="Close"
      >
    ...
    {#if $userStickers.length === 0}
      <div class="text-center text-gray-400 py-8">
        ...
        <button
          ...
          >{t("mainChat.addStickerButton")}</button
        >
      </div>
    {:else}
      <div class="grid grid-cols-4 gap-2">
        {#each $userStickers as sticker (sticker.id)}
          <div
            ...
            on:keydown={(e) => {
              if (e.key === "Enter" || e.key === " ") selectSticker(sticker);
            }}
            role="button"
            tabindex="0"
          ...
          >
            ...
            {/if}
            <div
              class="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-xs p-1 truncate rounded-b-lg"
            >
              {sticker.name}
            </div>
            <div
              class="absolute -top-1 -right-1 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 z-10"
            >
              <button
                on:click|stopPropagation={(e) => editName(sticker, e)}
                class="p-1 bg-blue-600 hover:bg-blue-700 rounded-full text-white"
              >
                <Edit3 class="w-3 h-3" />
              </button>
              <button
                on:click|stopPropagation={(e) => deleteSticker(sticker, e)}
