...
<script>
  ...
  import ImageResultModal from './ImageResultModal.svelte';
  ...
  async function testAppearancePrompt() {
    ...
    try {
      ...
      const result = await $stickerManager.naiClient.generateSticker(testCharacter, 'happy', {
...
<div class="space-y-6">
    ...
    <div>
        ...
        <div class="mt-2 flex justify-between items-center">
            ...
            <button on:click={testAppearancePrompt} disabled={isTesting} class="px-3 py-1 text-xs bg-purple-600 hover:bg-purple-700 text-white rounded-lg flex items-center gap-1 disabled:opacity-50">
                {#if isTesting}
