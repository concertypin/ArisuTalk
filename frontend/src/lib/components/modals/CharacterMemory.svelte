...
<details class="group border-t border-gray-700/50 pt-4">
    <summary class="flex items-center justify-between cursor-pointer list-none">
        <span class="text-base font-medium text-gray-200">{t("characterModal.memory")}</span>
        <slot name="chevron" />
    </summary>
    <div class="pt-4 space-y-2">
        {#each memories as memory, i}
            <div class="flex items-center gap-2">
                ...
                >
                <button on:click={() => deleteMemory(i)} class="p-2 text-gray-400 hover:text-red-400">
                    <Trash2 class="w-4 h-4" />
                </button>
            </div>
        {/each}
        <button on:click={addMemory} class="mt-3 text-sm text-blue-400 hover:text-blue-300 flex items-center gap-2">
            <PlusCircle class="w-4 h-4" /> {t("characterModal.addMemory")}
        </button>
    </div>
</details>
