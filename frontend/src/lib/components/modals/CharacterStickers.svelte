...
<script>
  ...
  import { Plus, CheckSquare, CheckCircle, Trash2, Edit3, Music, X, Sparkles } from 'lucide-svelte';
  import { stickerManager } from '../../stores/services';
  import { settings } from '../../stores/settings';
  import { editingCharacter } from '../../stores/character';
  import StickerProgressModal from './StickerProgressModal.svelte';
  import StickerPreviewModal from './StickerPreviewModal.svelte';
  ...
  async function handleFileChange(event) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const stickerName = file.name.split(".").slice(0, -1).join('.') || file.name;
        const newSticker = {
  ...
  function deleteSelected() {
    if (confirm(t('stickerPreview.confirmRemoveMultiple', { count: selectedStickers.length }))) {
  ...
  async function generateStickers() {
    ...
    if (!naiApiKey) {
      alert('NAI API 키가 설정되지 않았습니다. 설정에서 API 키를 입력해주세요.');
      return;
    }
    ...
    await get(stickerManager).generateBasicStickerSet(get(editingCharacter), (progress) => {
...
<details class="group border-t border-gray-700/50 pt-4">
    <summary class="flex items-center justify-between cursor-pointer list-none">
        <span class="text-base font-medium text-gray-200">{t("characterModal.sticker")}</span>
        <slot name="chevron" />
    </summary>
    <div class="pt-4 space-y-4">
        ...
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
                <button on:click={addStickers} class="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-sm flex items-center justify-center gap-2">
                    <Plus class="w-4 h-4" /> 
                    <span>{@html t("characterModal.addSticker")}</span>
                </button>
                <button on:click={generateStickers} class="py-2 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors text-sm flex items-center justify-center gap-2">
            ...
            {#if stickers.length > 0}
                <div class="flex items-center gap-2">
                    <button on:click={toggleSelectionMode} class="py-2 px-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm flex items-center gap-1">
                        <CheckSquare class="w-4 h-4" /> 
                        <span class="text-xs">{selectionMode ? t("characterModal.deselect") : t("characterModal.selectMode")}</span>
                    </button>
                    {#if selectionMode}
                        <button on:click={selectAll} class="py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-sm flex items-center gap-1">
                            <CheckCircle class="w-4 h-4" /> 
                            <span class="text-xs">{t("characterModal.selectAll")}</span>
                        </button>
                        <button on:click={deleteSelected} class="py-2 px-3 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm flex items-center gap-1" disabled={selectedStickers.length === 0}>
                            <Trash2 class="w-4 h-4" /> 
                            <span class="text-xs">{t("characterModal.deleteSelected").replace('0', selectedStickers.length)}</span>
                        </button>
                    {/if}
                </div>
            {/if}
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {#if stickers.length === 0}
                <div class="col-span-full text-center text-gray-400 text-sm py-4">
                    ...
                    <button on:click={addStickers} class="mt-2 text-blue-400 hover:text-blue-300 text-xs">
                ...
                {#each stickers as sticker, i (sticker.id)}
                    <div on:click={() => openPreviewModal(sticker, i)} class="sticker-item relative group aspect-square flex items-center justify-center p-1 rounded-lg bg-gray-700/50 cursor-pointer">
                        ...
                        {#if sticker.type?.startsWith('video')}
                            <video class="w-full h-full object-contain pointer-events-none" muted loop autoplay src={sticker.data}></video>
                        {:else if sticker.type?.startsWith('audio')}
                            <div class="w-full h-full flex items-center justify-center pointer-events-none">
                                <Music class="w-8 h-8 text-gray-400" />
                            </div>
                        {:else}
                            <img src={sticker.data} alt={sticker.name} class="w-full h-full object-contain pointer-events-none">
                        {/if}
                        <div class="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[10px] p-1 truncate rounded-b-lg pointer-events-none">
                            {sticker.name}
                        </div>

                        {#if !selectionMode}
                            <div class="absolute -top-1 -right-1 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 z-20">
                                <button on:click|stopPropagation={(e) => openPreviewModal(sticker, i)} class="p-1 bg-blue-600 hover:bg-blue-700 rounded-full text-white" title={t("characterModal.editStickerName")}>
                                    <Edit3 class="w-3 h-3" />
                                </button>
                                <button on:click|stopPropagation={(e) => stickers = stickers.filter(s => s.id !== sticker.id)} class="p-1 bg-red-600 hover:bg-red-700 rounded-full text-white" title={t("characterModal.deleteSticker")}>
...
<StickerProgressModal bind:progress={progressData} on:close={() => showProgressModal = false} on:cancel={() => get(stickerManager)?.cancelGeneration()} />
<StickerPreviewModal 
