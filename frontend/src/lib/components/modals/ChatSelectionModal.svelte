...
{#if isOpen && character}
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" on:click={closeModal}>
    <div class="bg-gray-800 rounded-2xl p-6 w-full max-w-sm mx-4" on:click|stopPropagation>
      ...
      <div class="max-h-60 overflow-y-auto space-y-2 pr-2">
        {#each characterChatRooms as chatRoom}
          ...
          <div class="chat-room-item p-3 rounded-lg cursor-pointer hover:bg-gray-700 transition-colors" on:click={() => selectChat(chatRoom.id)}>
