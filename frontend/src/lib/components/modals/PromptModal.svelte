<script>
  ...
  prompts.subscribe(value => {
      promptData = JSON.parse(JSON.stringify(value)); // Deep copy to avoid direct mutation
  });
...
<div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" on:click={() => isPromptModalVisible.set(false)}>
    <div class="bg-gray-800 rounded-2xl w-full max-w-4xl mx-4 flex flex-col max-h-[90vh]" on:click|stopPropagation>
        ...
        <div class="p-6 mt-auto border-t border-gray-700 shrink-0 flex flex-wrap justify-end gap-3">
            <button on:click={handleBackup} class="py-2 px-4 bg-gray-600 hover:bg-gray-500 text-white rounded-lg transition-colors text-sm flex items-center gap-2">
                <Download class="w-4 h-4" /> {t("promptModal.backupPrompts")}
            </button>
            <button on:click={handleRestore} class="py-2 px-4 bg-gray-600 hover:bg-gray-500 text-white rounded-lg transition-colors text-sm flex items-center gap-2">
                <Upload class="w-4 h-4" /> {t("promptModal.restorePrompts")}
            </button>
            <button on:click={handleReset} class="py-2 px-4 bg-red-600 hover:bg-red-500 text-white rounded-lg transition-colors text-sm flex items-center gap-2">
                <RefreshCw class="w-4 h-4" /> {t("promptModal.resetAllPrompts")}
            </button>
            <div class="flex-grow"></div>
            <button on:click={() => isPromptModalVisible.set(false)} class="py-2.5 px-4 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors">{t("common.cancel")}</button>
            <button on:click={handleSave} class="py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">{t("common.save")}</button>
        </div>
    </div>
</div>
{/if}
