<script lang="ts">
  ...
  import { X, LogIn, LogOut, UserCircle2, Loader2, AlertCircle } from "lucide-svelte";
  ...
  onMount(() => {
    ...
    (async () => {
      try {
        ...
        console.error("Failed to initialize Clerk for mobile auth modal", error);
      }
    })();
    ...
    >
      ...
      {#if isDisabled}
        <div class="flex flex-col items-center text-center gap-3 py-8">
          ...
          <p class="text-sm text-gray-300">{t("auth.authUnavailable")}</p>
        </div>
        ...
        <div class="space-y-4">
          ...
          >
            <div class="flex flex-col text-left">
              ...
              <span class="text-xs text-blue-200/70">{t("auth.openProfile")}</span>
            </div>
      ...
      {:else}
        <div class="space-y-4">
          ...
          {#if isError}
            <div class="flex flex-col items-center gap-2 text-center text-sm text-amber-300">
              <AlertCircle class="w-6 h-6" />
              <span>{state.error?.message ?? t("auth.retry")}</span>
              <button
