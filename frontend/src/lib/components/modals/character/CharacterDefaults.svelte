<script>
  ...
  import { ArrowLeft, User, Badge, BrainCircuit, MessageSquarePlus, Shuffle, Users } from 'lucide-svelte';
...
<div class="flex flex-col h-full">
  <header class="flex-shrink-0 px-4 py-4 bg-gray-900/80 flex items-center gap-2" style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
    ...
    <div>
        <h2 class="font-semibold text-white text-xl">{t("settings.characterDefaults")}</h2>
        <p class="text-sm text-gray-400">{t("settings.characterSubtitle")}</p>
    </div>
  </header>
  ...
  <div class="flex-1 overflow-y-auto p-4 space-y-6">
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <div class="space-y-4">
            <div>
                <label class="flex items-center text-sm font-medium text-gray-300 mb-2" for="settings-user-name-mobile">
                    <Badge class="w-4 h-4 mr-2" />
                    {t("settings.nameOrNickname")}
                </label>
                <input id="settings-user-name-mobile" type="text" placeholder={t("settings.yourNamePlaceholder")} bind:value={$settings.userName} class="w-full px-3 py-2 bg-gray-700 text-white rounded-lg border-0 focus:ring-2 focus:ring-blue-500/50 text-sm" />
                <p class="text-xs text-gray-400 mt-1">{t("settings.nameOrNicknameInfo")}</p>
            </div>
            <div>
                <label class="flex items-center text-sm font-medium text-gray-300 mb-2" for="settings-user-desc-mobile">
                    <BrainCircuit class="w-4 h-4 mr-2" />
                    {t("settings.selfIntroduction")}
                </label>
                <textarea id="settings-user-desc-mobile" placeholder={t("settings.selfIntroductionPlaceholder")} bind:value={$settings.userDescription} class="w-full px-3 py-2 bg-gray-700 text-white rounded-lg border-0 focus:ring-2 focus:ring-blue-500/50 resize-none text-sm" rows="4"></textarea>
                <p class="text-xs text-gray-400 mt-1">{t("settings.selfIntroductionInfo")}</p>
            </div>
        </div>
    </div>
            ...
            <div class="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                <div class="flex-1 pr-4">
                    <div class="flex items-center mb-1">
                        ...
                        <span class="text-sm font-medium text-white">{t("settings.proactiveChat")}</span>
                    </div>
                    <p class="text-xs text-gray-400">{t("settings.proactiveChatInfo")}</p>
                </div>
                <label class="relative inline-block w-10 h-5 cursor-pointer">
                    <input type="checkbox" bind:checked={$settings.proactiveChatEnabled} class="sr-only peer" />
                ...
                <div class="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                    <div class="flex-1 pr-4">
                        <div class="flex items-center mb-1">
                            ...
                            <span class="text-sm font-medium text-white">{t("settings.randomFirstMessage")}</span>
                        </div>
                        <p class="text-xs text-gray-400">{t("settings.randomFirstMessageInfo")}</p>
                    </div>
                    <label class="relative inline-block w-10 h-5 cursor-pointer">
                        <input type="checkbox" bind:checked={$settings.randomFirstMessageEnabled} class="sr-only peer" />
                        <div class="w-10 h-5 bg-gray-600 rounded-full peer-checked:bg-blue-600"></div>
                        <div class="absolute left-0.5 top-0.5 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-5"></div>
                    </label>
                </div>
