<script>
  ...
  import { isCharacterModalVisible, isSNSCharacterListModalVisible } from '../../../stores/ui';
  import { X, Image, Upload, Download, ChevronDown, Sparkles, MessageSquarePlus, Instagram } from 'lucide-svelte';
  ...
  editingCharacter.subscribe(char => {
    if (char) {
      ...
      if (!characterData.naiSettings) characterData.naiSettings = { qualityPrompt: 'masterpiece, best quality', autoGenerate: false };
      if (!characterData.hypnosis) characterData.hypnosis = { ...defaultHypnosis };
      ...
      characterData = { 
        ...
        memories: [],
        stickers: [],
        naiSettings: { qualityPrompt: 'masterpiece, best quality', autoGenerate: false },
        ...
        try {
          ...
          if (jsonString) {
            ...
            if (data.source === "PersonaChatAppCharacterCard") {
              characterData = { ...characterData, ...data, avatar: imageSrc };
    ...
    <div id="character-modal-panel" role="dialog" aria-modal="true" tabindex="0" aria-labelledby="character-modal-title" class="bg-gray-800 rounded-2xl w-full max-w-md mx-auto my-auto flex flex-col max-h-[90vh]" on:click|stopPropagation on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); } }}>
        ...
        <div class="flex items-center gap-2">
          {#if !isNew}
            <button on:click={() => isSNSCharacterListModalVisible.set(true)} class="p-2 md:p-2 hover:bg-gray-700 rounded-full transition-colors z-20" title={t("characterModal.openSNS")}>
                <Instagram class="w-6 h-6 md:w-5 md:h-5 text-gray-300" />
            </button>
          {/if}
      ...
      <div class="p-6 space-y-6 overflow-y-auto">
        ...
        <div class="flex items-center space-x-4">
            <div class="w-20 h-20 rounded-full bg-gray-700 flex items-center justify-center overflow-hidden shrink-0">
                {#if characterData.avatar}
                    <img src={characterData.avatar} alt="Avatar Preview" class="w-full h-full object-cover">
                {:else}
                    <Image class="w-8 h-8 text-gray-400" />
                {/if}
            </div>
            <div class="flex flex-col gap-2">
                <button on:click={selectAvatar} class="py-2 px-4 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm flex items-center justify-center gap-2">
                    <Image class="w-4 h-4" /> {t("characterModal.profileImage")}
                </button>
                <button on:click={importCard} class="py-2 px-4 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm flex items-center justify-center gap-2">
                    <Upload class="w-4 h-4" /> {t("characterModal.importContact")}
                </button>
                <button on:click={exportCard} class="py-2 px-4 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm flex items-center justify-center gap-2">
                    <Download class="w-4 h-4" /> {t("characterModal.shareContact")}
                </button>
            </div>
            <input type="file" accept="image/png,image/jpeg" bind:this={avatarInput} on:change={handleAvatarChange} class="hidden" />
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                ...
                <button on:click={generatePersona} disabled={isGeneratingPersona} class="py-1 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-xs flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    {:else}
                        <Sparkles class="w-3 h-3" /> AI {t("ui.generate")}
                    {/if}
                </button>
            </div>
        ...
        <CharacterAISettings 
            ...
            bind:naiQualityPrompt={characterData.naiSettings.qualityPrompt}
            bind:naiAutoGenerate={characterData.naiSettings.autoGenerate}
        />
        ...
        <details class="group border-t border-gray-700/50 pt-4">
            <summary class="flex items-center justify-between cursor-pointer list-none">
                ...
                <ChevronDown class="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
            </summary>
            <div class="pt-6 space-y-6">
                <div class="border-b border-gray-700/50 pb-6">
                    <label class="flex items-center justify-between text-sm font-medium text-gray-300 cursor-pointer">
                        <span class="flex items-center"><MessageSquarePlus class="w-4 h-4 mr-2" />{t("characterModal.proactiveToggle")}</span>
                        <div class="relative inline-block w-10 align-middle select-none">
                            <input type="checkbox" bind:checked={characterData.proactiveEnabled} id="character-proactive-toggle" class="absolute opacity-0 w-0 h-0 peer"/>
                ...
                <details class="group pt-2">
                    ...
                    <div class="pt-4 space-y-4">
                        <div>
                            <p class="text-sm font-medium text-gray-300 mb-2">{t("characterModalSlider.responseTime.description")}</p>
                            <input type="range" bind:value={characterData.responseTime} min="1" max="10" class="w-full">
                            <div class="flex justify-between text-xs text-gray-400 mt-1">
                                <span>{t("characterModalSlider.responseTime.low")}</span>
                                <span>{t("characterModalSlider.responseTime.high")}</span>
                            </div>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-300 mb-2">{t("characterModalSlider.thinkingTime.description")}</p>
                            <input type="range" bind:value={characterData.thinkingTime} min="1" max="10" class="w-full">
                            <div class="flex justify-between text-xs text-gray-400 mt-1">
                                <span>{t("characterModalSlider.thinkingTime.low")}</span>
                                <span>{t("characterModalSlider.thinkingTime.high")}</span>
                            </div>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-300 mb-2">{t("characterModalSlider.reactivity.description")}</p>
                            <input type="range" bind:value={characterData.reactivity} min="1" max="10" class="w-full">
                            <div class="flex justify-between text-xs text-gray-400 mt-1">
                                <span>{t("characterModalSlider.reactivity.low")}</span>
                                <span>{t("characterModalSlider.reactivity.high")}</span>
                            </div>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-300 mb-2">{t("characterModalSlider.tone.description")}</p>
                            <input type="range" bind:value={characterData.tone} min="1" max="10" class="w-full">
                            <div class="flex justify-between text-xs text-gray-400 mt-1">
                                <span>{t("characterModalSlider.tone.low")}</span>
                                <span>{t("characterModalSlider.tone.high")}</span>
                            </div>
                        </div>
                    </div>
                </details>

                <CharacterStickers bind:stickers={characterData.stickers}>
                    <ChevronDown slot="chevron" class="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
                </CharacterStickers>
      ...
      <div class="p-6 mt-auto border-t border-gray-700 shrink-0 flex justify-end space-x-3">
        <button on:click={closeModal} class="flex-1 md:flex-none py-2.5 px-6 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors">{t("common.cancel")}</button>
        <button on:click={saveCharacter} class="flex-1 md:flex-none py-2.5 px-6 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">{t("common.save")}</button>
      </div>
    </div>
  </div>
{/if}