<script lang="ts">
  ...
  const ensureService = async () => {
    if (!serviceModule) {
      serviceModule = await import("../../../services/phonebookService.ts");
    }
...
{#if isOpen}
  ...
  >
    <div class="phonebook-modal w-full max-w-3xl mx-4 bg-gray-900 rounded-2xl shadow-xl border border-white/5">
      <header class="flex items-center justify-between px-6 py-4 border-b border-white/5">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <RefreshCcw class={`w-5 h-5 ${isInitializing ? "animate-spin" : ""}`} />
          </button>
          <button
            class="p-2 rounded-full hover:bg-white/10 text-gray-300"
            on:click={closeModal}
      ...
      <section class="max-h-[70vh] overflow-y-auto px-6 py-4 space-y-4">
        ...
        {:else if entries.length === 0}
          <div class="flex flex-col items-center justify-center gap-3 py-16 text-center text-gray-400">
            <p>{t("phonebook.empty")}</p>
          </div>
        {:else}
          <ul class="space-y-3">
            {#each entries as entry (entry.id)}
              <li class="bg-gray-800/60 border border-white/5 rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="space-y-2 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    {#if entry.encrypted}
                      <span class="px-2 py-0.5 text-xs rounded-full bg-red-500/20 text-red-300 border border-red-500/40">
                  ...
                  <div class="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                    {#if entry.author}
                      <span>{t("phonebook.byAuthor", { author: entry.author })}</span>
                    {/if}
                    {#if typeof entry.downloadCount === "number"}
                      <span>{t("phonebook.downloads", { count: entry.downloadCount.toString() })}</span>
                    {/if}
                    {#if typeof entry.uploadedAt === "number"}
                      <span>{t("phonebook.uploaded", { date: new Date(entry.uploadedAt).toLocaleString() })}</span>
                    {/if}
                  </div>
                </div>
                <div class="flex items-center gap-2 self-stretch md:self-center">
                  <button
                    ...
                    disabled={entry.encrypted || Boolean(busyEntryId)}
                  >
                    {#if busyEntryId === entry.id}
                      <Loader2 class="w-4 h-4 animate-spin" />
                      <span>{t("phonebook.importing")}</span>
                    {:else}
