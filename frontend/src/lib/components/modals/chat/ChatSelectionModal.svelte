...
{#if isOpen && character}
    ...
    >
        <div
            ...
            aria-modal="true"
            tabindex="0"
            aria-labelledby="chat-selection-modal-title"
            class="bg-gray-800 rounded-2xl p-6 w-full max-w-sm mx-4"
        ...
        >
            <div class="flex items-center mb-4">
                ...
                <h3
                    id="chat-selection-modal-title"
                    class="text-lg font-semibold text-white ml-4"
                >
                    {character.name}
                </h3>
                <button
                    on:click={closeModal}
                    class="ml-auto p-2 rounded-full hover:bg-gray-700"
                >
                    <X class="w-5 h-5 text-gray-300" />
                </button>
            </div>
            <p class="text-sm text-gray-300 mb-4">
                {t("modal.selectChat.message")}
            </p>
            <div class="max-h-60 overflow-y-auto space-y-2 pr-2">
                {#each characterChatRooms as chatRoom}
                    ...
                    <button
                        class="chat-room-item p-3 rounded-lg hover:bg-gray-700 transition-colors w-full text-left"
                        on:click={() => selectChat(chatRoom.id)}
                    >
                        <div class="flex justify-between items-center">
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-white truncate">
                                    {chatRoom.name ||
                                        t("sidebar.defaultChatName")}
                                </p>
                            ...
                            {#if unreadCount > 0}
                                <span
                                    class="bg-red-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full leading-none ml-2"
                                    >{unreadCount}</span
                                >
                            {/if}
                        </div>
            ...
            <button
                ...
                class="w-full mt-4 py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors flex items-center justify-center gap-2"
            >
