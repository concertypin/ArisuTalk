  ...
  function createGroupChat() {
    ...
    const newGroupChat = {
      ...
      settings: {
        ...
        responseDelay: 3000,
        participantSettings: selectedParticipantIds.reduce((acc, id) => {
          acc[id] = { isActive: true, responseProbability: 0.9, characterRole: 'normal' };
          return acc;
        }, {}),
      },
    };
  ...
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" on:click={closeModal}>
    <div class="bg-gray-800 rounded-2xl w-full max-w-md mx-auto my-auto flex flex-col max-h-[90vh]" on:click|stopPropagation>
          ...
          <div class="space-y-2 max-h-60 overflow-y-auto character-list-scrollbar pr-2">
            {#each $characters as character (character.id)}
              <label class="flex items-center space-x-3 p-3 hover:bg-gray-700 rounded-lg cursor-pointer group">
                <input type="checkbox" bind:group={selectedParticipantIds} value={character.id} class="w-4 h-4 text-blue-600 bg-gray-600 border-gray-500 rounded focus:ring-blue-500 focus:ring-offset-0">
      ...
      <div class="p-6 mt-auto border-t border-gray-700 shrink-0 flex justify-end space-x-3">
        ...
        <button on:click={createGroupChat} class="flex-1 py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-center">{t("common.create")}</button>
      </div>
    </div>
  </div>
{/if}
