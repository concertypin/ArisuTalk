<script>
  ...
  editingGroupChat.subscribe(chat => {
    if (chat) {
      participants = chat.participantIds.map(id => $characters.find(c => c.id === id)).filter(Boolean);
      chatName = chat.name;
      responseFrequency = Math.round((chat.settings.responseFrequency || 0.5) * 100);
      ...
      participantSettings = JSON.parse(JSON.stringify(chat.settings.participantSettings || {}));

      // Ensure all participants have a settings object
      for (const p of participants) {
        if (!participantSettings[p.id]) {
          participantSettings[p.id] = { isActive: true, responseProbability: 0.9, characterRole: 'normal' };
        }
      }
    }
  });
...
{#if $editingGroupChat}
  ...
  >
    <div role="dialog" aria-modal="true" tabindex="0" aria-labelledby="edit-group-chat-title" class="bg-gray-800 rounded-2xl w-full max-w-2xl mx-auto my-auto flex flex-col max-h-[90vh]" on:click|stopPropagation on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); } }}>
        ...
        <div class="p-6 space-y-6 overflow-y-auto">
            ...
            <div class="space-y-4 pt-4 border-t border-gray-700/50">
                ...
                <div>
                    ...
                    >
                    <p class="text-xs text-gray-400 mt-1">{t("groupChat.responseIntervalInfo")}</p>
                </div>
            </div>

            <!-- Participant Settings -->
            <div class="space-y-4 pt-4 border-t border-gray-700/50">
                <h3 class="text-lg font-semibold text-white">{t("groupChat.individualCharacterSettings")}</h3>
                <div class="space-y-4">
                        ...
                        <div class="bg-gray-700/50 p-4 rounded-lg">
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" bind:checked={settings.isActive} id={`active-${participant.id}`} class="w-4 h-4 text-blue-600 bg-gray-600 border-gray-500 rounded focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                        class="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer slider"
                                        on:input={(e) => settings.responseProbability = parseFloat(e.target.value)}
                                    >
                                </div>
                                <div>
        ...
        <div class="p-6 mt-auto border-t border-gray-700 shrink-0 flex justify-end space-x-3 sticky bottom-0 bg-gray-800 z-10">
            <button on:click={closeModal} class="flex-1 md:flex-none py-2.5 px-6 bg-gray-600 hover:bg-gray-500 text-white rounded-lg transition-colors text-center">{t("common.cancel")}</button>
            <button on:click={saveChanges} class="flex-1 md:flex-none py-2.5 px-6 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-center">{t("common.save")}</button>
        </div>
    </div>
  </div>
{/if}