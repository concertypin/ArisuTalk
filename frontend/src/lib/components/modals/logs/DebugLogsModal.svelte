            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                    ...
                    <p class="text-gray-400 text-sm mt-1">
                        {t("debugLogs.totalLogItems")} {$debugLogs.length}/{maxLogs}{t("debugLogs.maxLogItems")} |
                        {t("debugLogs.logCollectionStatus")} {$settings.enableDebugLogs ? t("debugLogs.enabled") : t("debugLogs.disabled")}
                    </p>
                </div>
                <div class="flex flex-wrap justify-end gap-2 w-full sm:w-auto sm:flex-nowrap">
                    {#if $debugLogs.length > 0}
                        <button on:click={exportDebugLogs} class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg flex items-center gap-2 text-sm">
                            <Download class="w-4 h-4" />
                            {t("debugLogs.export")}
                        </button>
                        <button on:click={handleClearLogs} class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg flex items-center gap-2 text-sm">
                            <Trash2 class="w-4 h-4" />
                            {t("debugLogs.clearAll")}
                        </button>
                    {/if}
                    <button on:click={() => isDebugLogModalVisible.set(false)} class="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-lg text-sm">
                        {t("debugLogs.close")}
                    </button>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-hidden">
            {#if $debugLogs.length === 0}
                <div class="flex flex-col items-center justify-center h-full text-gray-400">
                    ...
                    <h3 class="text-lg font-medium mb-2">{t("debugLogs.noLogs")}</h3>
                    <p class="text-sm text-center">
                        {@html $settings.enableDebugLogs ? t("debugLogs.noLogsCollected") : t("debugLogs.logsDisabled")}
                    </p>
                </div>
            {:else}
                <div class="p-6 h-full overflow-auto">
                    <div class="space-y-4">
                        {#each $debugLogs.slice().sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()) as log (log.id)}
