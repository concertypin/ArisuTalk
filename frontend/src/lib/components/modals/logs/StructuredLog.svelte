  ...
  const getLogTypeColor = (type, data) => {
    if (type === "structured") {
      ...
      switch (chatType) {
        ...
        case "open": return "text-purple-400";
        case "nai_generation": return "text-pink-400";
        case "sns_generation": return "text-pink-400";
...
<div class="border border-gray-600 rounded-lg bg-gray-750">
    ...
    <div class="divide-y divide-gray-600">
        {#if log.data?.personaInput}
            <details class="group">
                <summary class="p-3 cursor-pointer text-sm font-medium text-blue-300 hover:bg-gray-700 group-open:bg-gray-700 flex items-center">
                    <ChevronRight class="w-4 h-4 mr-2 group-open:transform group-open:rotate-90 transition-transform" />
                    Persona Input
                </summary>
                <div class="p-3 bg-gray-800 text-xs">
                    <pre class="text-gray-300 overflow-x-auto">{JSON.stringify(log.data.personaInput, null, 2)}</pre>
                </div>
            </details>
        {/if}
        
        <details class="group">
            <summary class="p-3 cursor-pointer text-sm font-medium text-yellow-300 hover:bg-gray-700 group-open:bg-gray-700 flex items-center">
                <ChevronRight class="w-4 h-4 mr-2 group-open:transform group-open:rotate-90 transition-transform" />
                Chat Prompt
            </summary>
            <div class="p-3 bg-gray-800 text-xs">
                <pre class="text-gray-300 overflow-x-auto whitespace-pre-wrap">{promptContent || 'Loading...'}</pre>
            </div>
        </details>
        
        {#if log.data?.outputResponse}
            <details class="group">
                <summary class="p-3 cursor-pointer text-sm font-medium text-green-300 hover:bg-gray-700 group-open:bg-gray-700 flex items-center">
                    <ChevronRight class="w-4 h-4 mr-2 group-open:transform group-open:rotate-90 transition-transform" />
                    Character Response
                </summary>
                <div class="p-3 bg-gray-800 text-xs">
                    <pre class="text-gray-300 overflow-x-auto">{JSON.stringify(log.data.outputResponse, null, 2)}</pre>
                </div>
            </details>
        {/if}
        ...
        {#if log.data?.parameters}
            <details class="group">
                <summary class="p-3 cursor-pointer text-sm font-medium text-purple-300 hover:bg-gray-700 group-open:bg-gray-700 flex items-center">
                    <ChevronRight class="w-4 h-4 mr-2 group-open:transform group-open:rotate-90 transition-transform" />
                    Parameters
                </summary>
                <div class="p-3 bg-gray-800 text-xs">
                    <pre class="text-gray-300 overflow-x-auto">{JSON.stringify(log.data.parameters, null, 2)}</pre>
                </div>
            </details>
        {/if}
        ...
        {#if log.data?.metadata}
            <details class="group">
                <summary class="p-3 cursor-pointer text-sm font-medium text-gray-300 hover:bg-gray-700 group-open:bg-gray-700 flex items-center">
                    <ChevronRight class="w-4 h-4 mr-2 group-open:transform group-open:rotate-90 transition-transform" />
                    Metadata
                </summary>
                <div class="p-3 bg-gray-800 text-xs">
                    <pre class="text-gray-300 overflow-x-auto">{JSON.stringify(log.data.metadata, null, 2)}</pre>
                </div>
            </details>
        {/if}
    </div>
