...
{#if tooltipVisible}
  ...
  >
    ...
    <p>
      Magic Patterns are special patterns that can be used in the prompt to
      access character properties and current chatting log.
    </p>
    <p>
      Patterns start with <code>{@html "{|"}</code> and ends with
      <code>{@html "|}"}</code>. Inner text is command. Multi-line is supported.
    </p>
    <p>
      All patterns are interpreted, executed on sandboxed JavaScript engine,
      which means you can use any valid JavaScript syntax as long as it doesn't
      access outside of the sandbox.
    </p>
    <p>
      After you write the pattern, you MUST use <code>return</code> statement to
      make the pattern return something! If you don't, it will return empty string.
    </p>
    <p>
    ...
    <ul class="list-disc list-inside">
      ...
      <li>
        <code>chat(a, b)</code>: Function to access current chatting log from
        <code>a</code>-th to <code>b</code>-th.
      </li>
      <li>
        <code>sessionStorage</code>: Same as <code>window.sessionStorage</code>.
      </li>
    </ul>
  </div>
{/if}
