...
<div class="bg-gray-900/50 rounded-lg p-4">
    ...
    <textarea bind:value class="w-full h-80 p-3 bg-gray-700 text-white rounded-lg text-sm font-mono"></textarea>
</div>
...
>
    ...
    <p>Patterns start with <code>{@html '{|'}</code> and ends with <code>{@html '|}'}</code>. Inner text is command. Multi-line is supported.</p>
    ...
    <p>There are no escape mechanism, and all <code>{@html '{|'}</code> and <code>{@html '|}'}</code> patterns will be treated as magic patterns.</p>
