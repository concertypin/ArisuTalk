...
{#if isOpen}
    <button
        ...
        class="modal-backdrop fixed inset-0 bg-black/60 z-50"
        on:click={closeModal}
    ></button>
    <div
        class="search-modal-container p-4 pt-16 md:pt-24 fixed inset-0 z-50 overflow-y-auto pointer-events-none"
    >
        <div
            class="search-modal-content bg-gray-800 rounded-2xl shadow-lg w-full max-w-lg mx-auto flex flex-col max-h-[85vh] pointer-events-auto"
            bind:this={modalContentEl}
        >
            <header
                class="p-4 flex-shrink-0 flex items-center gap-4 border-b border-gray-700"
            >
                ...
                <input
                    ...
                    class="flex-1 w-full bg-transparent text-white placeholder-gray-500 focus:outline-none text-lg"
                />
                <button
                    on:click={closeModal}
                    class="p-2 -mr-2 rounded-full hover:bg-gray-700"
                >
