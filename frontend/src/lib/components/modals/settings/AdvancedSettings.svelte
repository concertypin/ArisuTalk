<script>
  ...
  import { ArrowLeft, Bug, Activity, BarChart3, Trash2, Gauge, Clock, FlaskConical, AlertTriangle, Info, Key } from 'lucide-svelte';
...
<div class="flex flex-col h-full">
  <header class="flex-shrink-0 px-4 py-4 bg-gray-900/80 flex items-center gap-2" style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
    ...
    <div>
        <h2 class="font-semibold text-white text-xl">{t("settings.advancedSettings")}</h2>
        <p class="text-sm text-gray-400">{t("settings.advancedSubtitle")}</p>
    </div>
  </header>
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <button on:click={() => isDebugLogModalVisible.set(true)} class="w-full py-3 px-4 bg-blue-600/80 hover:bg-blue-600 text-white rounded-lg flex items-center justify-center gap-2 text-sm">
                    <BarChart3 class="w-4 h-4" />
                    {t("settings.viewLogs")}
                </button>
                <button on:click={handleClearLogs} class="w-full py-3 px-4 bg-red-600/80 hover:bg-red-600 text-white rounded-lg flex items-center justify-center gap-2 text-sm">
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <div class="bg-orange-900/20 border border-orange-500/30 rounded-lg p-3">
            <div class="flex items-start gap-2">
                <AlertTriangle class="w-4 h-4 text-orange-400 shrink-0 mt-0.5" />
                <div class="text-xs text-gray-300">
                    <p class="font-medium text-orange-300 mb-1">{t("settings.warningNote")}</p>
                    <p class="text-gray-400">{t("settings.experimentalWarning")}</p>
                </div>
            </div>
        </div>
    </div>
