<script>
  ...
  import { ArrowLeft, Type, Info, Globe, Palette, Moon, Clock, Zap, Check } from 'lucide-svelte';
...
<div class="flex flex-col h-full">
  <header class="flex-shrink-0 px-4 py-4 bg-gray-900/80 flex items-center gap-2" style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
    ...
    <div>
        <h2 class="font-semibold text-white text-xl">{t("settings.appearanceSettings")}</h2>
        <p class="text-sm text-gray-400">{t("settings.appearanceSubtitle")}</p>
    </div>
  </header>
  ...
  <div class="flex-1 overflow-y-auto p-4 space-y-6">
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <div class="space-y-3">
            <button on:click={() => handleLanguageChange('ko')} class="w-full p-3 rounded-lg border transition-all duration-200 flex items-center gap-3 {currentLanguage === 'ko' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600'}">
                <div class="text-xl">🇰🇷</div>
                <div class="text-left flex-1">
                    <div class="font-medium text-sm">{t("settings.languageKorean")}</div>
                    <div class="text-xs opacity-75">한국어</div>
                </div>
                {#if currentLanguage === 'ko'}<Check class="w-5 h-5 text-blue-400" />{/if}
            </button>
            <button on:click={() => handleLanguageChange('en')} class="w-full p-3 rounded-lg border transition-all duration-200 flex items-center gap-3 {currentLanguage === 'en' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600'}">
                <div class="text-xl">🇺🇸</div>
                <div class="text-left flex-1">
                    <div class="font-medium text-sm">{t("settings.languageEnglish")}</div>
                    <div class="text-xs opacity-75">English</div>
                </div>
                {#if currentLanguage === 'en'}<Check class="w-5 h-5 text-blue-400" />{/if}
            </button>
            <div class="bg-gray-700/50 rounded-lg p-2">
                <p class="text-xs text-gray-300/80 flex items-start">
                    <Info class="w-3 h-3 inline mr-1.5 shrink-0 mt-0.5" />
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <div class="bg-gray-700/50 rounded-lg p-4 text-center">
            ...
            <p class="text-gray-300 text-sm font-medium">{t("settings.darkTheme")}</p>
            <p class="text-xs text-gray-400 mt-1">{t("settings.currentTheme")}</p>
        </div>
    </div>
