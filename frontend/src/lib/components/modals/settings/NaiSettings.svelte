<script>
  ...
  import { ArrowLeft, Image, Cpu, UserPlus, Settings, Edit, Smile, Download, BarChart3, HelpCircle, Eye, EyeOff, Plus, Check, X, RefreshCw, Users, Trash2 } from 'lucide-svelte';
  ...
  let maxAdditionalDelaySeconds = ($settings.naiSettings?.maxAdditionalDelay ?? 10000) / 1000;
...
<div class="flex flex-col h-full">
  <header class="flex-shrink-0 px-4 py-4 bg-gray-900/80 flex items-center gap-2" style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
    ...
    <div>
        <h2 class="font-semibold text-white text-xl">{t("settings.naiSettings")}</h2>
  ...
  <div class="flex-1 overflow-y-auto p-4 space-y-6">
    ...
    <div class="bg-gray-800 rounded-xl p-4">
      ...
      <div class="space-y-3">
        ...
        <div>
          ...
          <div class="text-xs text-gray-400 mt-1">{t("naiSettings.imageSizeHelp")}</div>
        </div>
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input bind:checked={$settings.naiSettings.sm} type="checkbox" class="w-4 h-4 text-purple-600 bg-gray-700 border-gray-600 rounded focus:ring-purple-500 focus:ring-2">
                    <span class="text-xs font-medium text-gray-300">{t("naiSettings.smeaEnable")}</span>
                </label>
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input bind:checked={$settings.naiSettings.sm_dyn} type="checkbox" class="w-4 h-4 text-purple-600 bg-gray-700 border-gray-600 rounded focus:ring-purple-500 focus:ring-2">
                    <span class="text-xs font-medium text-gray-300">{t("naiSettings.smeaDynEnable")}</span>
                </label>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <div>
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        <h4 class="text-md font-semibold text-white mb-3 flex items-center justify-between">
            ...
            <button on:click={() => isEditingNaiList = !isEditingNaiList} class="px-2 py-1 text-xs bg-blue-600 hover:bg-blue-700 text-white rounded-md flex items-center gap-1">
                <Edit class="w-3 h-3" />
        ...
        {#if !isEditingNaiList}
            <div class="grid grid-cols-2 gap-2 mb-2">
                {#each ($settings.naiSettings.naiGenerationList || DEFAULT_EMOTIONS) as item}
                    ...
                    <div class="bg-gray-700/50 rounded-md px-2 py-1 text-center">
        ...
        {:else}
            <div class="space-y-3">
                <div class="space-y-2 max-h-32 overflow-y-auto">
                    {#each $settings.naiSettings.naiGenerationList as item, index}
                        ...
                        <div class="flex items-center gap-2 bg-gray-700/50 p-2 rounded-lg">
                            <span class="flex-1 text-xs text-gray-300 truncate">{displayText}</span>
                            <button on:click={() => deleteNaiItem(index)} class="p-1.5 bg-red-600/80 hover:bg-red-700 rounded-md">
                ...
                <div class="flex gap-2 pt-2 border-t border-gray-700">
                    <button on:click={saveNaiList} class="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex items-center justify-center gap-2 text-xs">
                        <Check class="w-4 h-4" />{t('g.save')}
                    </button>
                    <button on:click={resetNaiList} class="py-2 px-3 bg-red-600/80 hover:bg-red-700 text-white rounded-lg flex items-center justify-center gap-2 text-xs">
                        <RefreshCw class="w-4 h-4" />{t('g.reset')}
                    </button>
                </div>
            </div>
        {/if}
        <p class="text-xs text-gray-400 mt-2">{t("naiSettings.emotionStickersHelp")}</p>
    </div>
    ...
    <div class="bg-gray-800 rounded-xl p-4">
        ...
        <button on:click={generateAllStickers} class="w-full py-2 px-4 bg-green-600 hover:bg-green-700 text-white rounded-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm" disabled={!$settings.naiSettings.apiKey}>
            <Users class="w-4 h-4" />
            {t("naiSettings.generateAllCharacters")}
        </button>
        {#if !$settings.naiSettings.apiKey}
            <p class="text-xs text-yellow-400 text-center mt-2">{t("naiSettings.batchGenerationDisabled")}</p>
        {:else}
            <p class="text-xs text-gray-400 text-center mt-2">{t("naiSettings.batchGenerationHelp")}</p>
        {/if}
    </div>

    <!-- Help Guide -->
    <div class="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4">
        <h4 class="text-md font-semibold text-blue-300 mb-2 flex items-center">
