        ...
        <div class="space-y-4">
            <div class="relative">
                ...
                <div class="w-full flex justify-between text-xs text-gray-400 mt-1">
                    <span>80%</span>
                    <span class="absolute left-1/3 -translate-x-1/2">100% ({t("system.default")})</span>
                    <span>140%</span>
                </div>
            </div>
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <button 
                    ...
                    class="p-4 rounded-xl border-2 transition-all duration-200 flex items-center gap-3 {currentLanguage === 'ko' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-600/30 border-gray-600 text-gray-300 hover:bg-gray-600/50 hover:border-gray-500'}"
                >
                    <div class="text-2xl">🇰🇷</div>
                    <div class="text-left">
                        <div class="font-medium">{t("settings.languageKorean")}</div>
                        <div class="text-xs opacity-75">한국어</div>
                    </div>
                </button>
                <button 
                    on:click={() => handleLanguageChange('en')}
                    class="p-4 rounded-xl border-2 transition-all duration-200 flex items-center gap-3 {currentLanguage === 'en' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-600/30 border-gray-600 text-gray-300 hover:bg-gray-600/50 hover:border-gray-500'}"
                >
                    <div class="text-2xl">🇺🇸</div>
                    <div class="text-left">
                        <div class="font-medium">{t("settings.languageEnglish")}</div>
