...
<script>
  ...
  import { HardDrive, Download, Upload, Info, FileText, FilePenLine, Camera, History, Mouse, AlertTriangle, Trash2 } from 'lucide-svelte';
...
<div class="space-y-6">
    ...
    <div class="bg-gray-700/30 rounded-xl p-6">
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button on:click={backupData} class="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
                    <Download class="w-4 h-4" />
                    {t("settings.backupData")}
                </button>
                <button on:click={() => restoreFileInput.click()} class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
            ...
            <div class="bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
                <div class="flex items-start gap-2">
                    ...
                    <div class="text-xs text-gray-300">
                        <p class="font-medium text-blue-400 mb-1">{t("settings.backupInfoTitle")}</p>
    ...
    <div class="bg-gray-700/30 rounded-xl p-6">
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button on:click={prompts.backup} class="w-full py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
                    <Download class="w-4 h-4" />
                    {t("settings.promptBackup")}
                </button>
                <button on:click={prompts.restore} class="w-full py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
                    <Upload class="w-4 h-4" />
        ...
        <div class="space-y-4">
            ...
            <div id="snapshots-list" class="space-y-2">
                <div class="bg-gray-600/20 rounded-lg p-4">
                    <h6 class="text-sm font-medium text-gray-300 mb-3">{t("settings.savedSnapshots")}</h6>
        ...
        <div class="space-y-4">
            <div class="bg-red-900/30 border border-red-500/40 rounded-lg p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <h5 class="font-medium text-red-400 mb-2">{t("settings.resetAllData")}</h5>
                        <div class="text-xs text-gray-300 space-y-1">
                            <p>{t("settings.resetAllDataInfo")}</p>
                            <ul class="list-disc list-inside space-y-0.5 ml-2">
                                <li>{t("settings.resetDataList.allCharacters")}</li>
                                <li>{t("settings.resetDataList.allChatHistory")}</li>
                                <li>{t("settings.resetDataList.userSettings")}</li>
                                <li>{t("settings.resetDataList.stickerData")}</li>
                                <li>{t("settings.resetDataList.debugLogs")}</li>
                            </ul>
                            <p class="text-red-300 font-medium mt-2">{t("settings.resetWarning")}</p>
                        </div>
                    </div>
                </div>
                <button on:click={resetAllData} class="w-full mt-4 py-3 px-4 bg-red-600 hover:bg-red-500 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
...
<!-- 숨겨진 파일 입력 요소들 -->
<input bind:this={restoreFileInput} on:change={handleRestoreFile} type="file" id="restore-file-input" accept=".json" style="display: none;">
