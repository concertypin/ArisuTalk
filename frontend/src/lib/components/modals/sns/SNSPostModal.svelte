  ...
  >
    ...
    >
      <div class="flex justify-between items-center mb-4">
        <h2 id="sns-post-modal-title" class="text-lg font-semibold text-white">
          {editingPost.isNew ? t("sns.createPost") : t("sns.editPost")}
        </h2>
      ...
      <div class="mb-4">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <div class="text-xs text-gray-400">
              {editingPost.isSecret ? t("sns.secretAccountWarning") : ""}
            </div>
          </div>
        </div>
            ...
            <div class="grid grid-cols-4 gap-2">
              {#if character.stickers && character.stickers.length > 0}
                {#each character.stickers as sticker (sticker.id)}
                  ...
                  >
                    <img
                      src={sticker.data || sticker.dataUrl}
                      alt={sticker.name}
        ...
        <div class="mt-3">
          <label for="sns-tags" class="text-sm text-gray-400 mb-1 block"
            >{t("sns.tags")}</label
          >
        ...
        <div class="mt-3">
          <label for="sns-access-level" class="text-sm text-gray-400 mb-1 block"
          ...
          >
            {#if editingPost.isSecret}
              <option value="secret-public"
                >{t("sns.secretPublic")} - {t("sns.secretPublicDesc")}</option
              >
              <option value="secret-private"
                >{t("sns.secretPrivate")} - {t("sns.secretPrivateDesc")}</option
              >
            {:else}
              <option value="main-public"
                >{t("sns.mainPublic")} - {t("sns.mainPublicDesc")}</option
              >
              <option value="main-private"
                >{t("sns.mainPrivate")} - {t("sns.mainPrivateDesc")}</option
              >
            {/if}
          </select>
        </div>
        ...
        <div class="mt-3">
          <label for="sns-importance" class="text-sm text-gray-400 mb-1 block"
