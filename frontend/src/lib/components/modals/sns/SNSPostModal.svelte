...
<script>
  ...
  $: {
    if (editingPost) {
      ...
      accessLevel = editingPost.accessLevel || (editingPost.isSecret ? 'secret-public' : 'main-public');
  ...
  <div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" on:click={closeModal}>
    <div class="bg-gray-800 rounded-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" on:click|stopPropagation>
      ...
      <div class="mb-4">
        ...
        {#if stickerId}
          <div class="mb-4 bg-gray-700 rounded-lg p-4">
            <div class="flex justify-between items-center mb-2">
              ...
              <button on:click={() => stickerId = null} class="text-red-400 hover:text-red-300 text-sm">
                {t("common.remove")}
              </button>
            </div>
            <div class="bg-gray-600 rounded p-3">
              <img src={getStickerUrl(stickerId)} alt="Selected sticker" class="max-w-full max-h-32 mx-auto object-contain" />
            </div>
          </div>
        {/if}
        
        <div class="mb-3">
          <button on:click={() => showStickerPanel = !showStickerPanel} class="w-full px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg text-sm transition-colors">
            <Image class="w-4 h-4 inline mr-2" />
            {stickerId ? t("sns.changeSticker") : t("sns.selectSticker")}
          </button>
        </div>
        
        {#if showStickerPanel}
          <div class="mb-4 bg-gray-700 rounded-lg p-3 max-h-48 overflow-y-auto">
            <div class="grid grid-cols-4 gap-2">
              {#if character.stickers && character.stickers.length > 0}
                {#each character.stickers as sticker (sticker.id)}
                  <button on:click={() => { stickerId = sticker.id; showStickerPanel = false; }} class="p-2 bg-gray-600 hover:bg-gray-500 rounded transition-colors">
                    <img src={sticker.data || sticker.dataUrl} alt={sticker.name} class="w-full h-16 object-contain" />
        ...
        <div class="mt-3">
          ...
          <select bind:value={accessLevel} class="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            {:else}
              <option value="main-public">{t("sns.mainPublic")} - {t("sns.mainPublicDesc")}</option>
              <option value="main-private">{t("sns.mainPrivate")} - {t("sns.mainPrivateDesc")}</option>
            {/if}
          </select>
        </div>
      ...
      <div class="flex gap-3 justify-end">
        <button on:click={closeModal} class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-500 transition-colors">
          {t("common.cancel")}
        </button>
        <button on:click={savePost} class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors">
