...
<div class="bg-gray-800/90 backdrop-blur-sm rounded-xl overflow-hidden relative group hover:shadow-lg transition-all duration-200 border border-gray-700/50" data-post-id={post.id}>
  {#if character?.hypnosis?.enabled && character?.hypnosis?.sns_edit_access}
    <div class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-200 z-20 flex space-x-2">
      <button on:click={handleEdit} class="p-2 rounded-full bg-gray-800 hover:bg-blue-600 transition-colors" title="포스트 편집">
        <Edit3 class="w-4 h-4 text-gray-300" />
      </button>
      <button on:click={handleDelete} class="p-2 rounded-full bg-gray-800 hover:bg-red-600 transition-colors" title="포스트 삭제">
      ...
      {#if post.stickerId}
        <div class="mb-6 flex justify-center">
          <div class="bg-gray-700 rounded-xl p-6 max-w-sm w-full">
            <img src={getStickerUrl(character, post.stickerId)} alt="Sticker" class="w-full h-auto max-w-xs max-h-60 object-contain mx-auto" />
          </div>
        </div>
      {/if}
    ...
    {#if post.tags && post.tags.length > 0}
      <div class="mt-3 flex flex-wrap gap-1">
        {#each post.tags as tag}
          <span class="px-2 py-1 bg-blue-900/30 text-blue-400 text-xs rounded-full">
            #{tag}
          </span>
        {/each}
      </div>
