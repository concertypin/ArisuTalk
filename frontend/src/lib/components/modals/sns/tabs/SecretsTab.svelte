  ...
  $: {
    if (character && character.snsPosts) {
      secretPosts = character.snsPosts
        .filter(post => post.access_level && (post.access_level.includes('private') || post.access_level.includes('secret')))
        .map(post => {
          const hasAccess = checkSNSAccess(character, post.access_level || 'public', get(characterStateStore)[character.id]);
          return {
            ...post,
...
{:else}
  <div class="p-4 space-y-6">
    ...
    {#each secretPosts as post (post.id)}
      <SNSPost {post} {character} {isSecretMode} on:edit={(e) => window.personaApp.editSNSPost(e.detail.characterId, e.detail.postId)} on:delete={(e) => window.personaApp.deleteSNSPost(e.detail.characterId, e.detail.postId)} />
    {/each}
  </div>
{/if}
