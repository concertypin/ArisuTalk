    ...
    <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-medium text-gray-900">
          {t("stickerProgress.title", {
            name:
              progress.character?.name || t("stickerProgress.defaultCharacter"),
          })}
        </h3>
        <div>
      ...
      <div class="mb-4 p-3 rounded-lg {getStatusBgColor(progress.status)}">
        <div class="flex items-center">
          {#if progress.status === "generating"}
            <RefreshCw
              class="w-5 h-5 animate-spin {getStatusTextColor(progress.status)}"
            />
          {:else if progress.status === "completed"}
            <Check class="w-5 h-5 {getStatusTextColor(progress.status)}" />
          {:else if progress.status === "error"}
            <AlertCircle
              class="w-5 h-5 {getStatusTextColor(progress.status)}"
            />
          ...
          <div class="ml-3">
            <p
              class="text-sm font-medium {getStatusTextColor(progress.status)}"
            >
