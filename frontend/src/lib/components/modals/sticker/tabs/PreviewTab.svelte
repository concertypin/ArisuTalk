...
{#if sticker}
  <div class="space-y-4">
    ...
    <div class="text-xs text-gray-400 space-y-1">
      ...
      {#if sticker.size}
        <div>{t("stickerPreview.fileSize")}: {formatStickerSize(sticker.size)}</div>
      {/if}
      {#if sticker.createdAt}
        <div>{t("stickerPreview.dateAdded")}: {new Date(sticker.createdAt).toLocaleString()}</div>
      {/if}
    </div>

    <div class="flex gap-3 mt-6">
      <button on:click={save} class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-sm font-medium">
        {t("common.save")}
      </button>
    </div>
  </div>
