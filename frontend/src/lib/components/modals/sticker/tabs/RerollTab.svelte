...
{:else}
  <div class="space-y-4">
    {#if rerollResult}
      <div id="reroll-comparison">
        <h4 class="text-sm font-medium text-white mb-2">
          {t("stickerPreview.imageComparison")}
        </h4>
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-gray-700 rounded-lg p-3">
            <div class="text-xs text-gray-300 mb-2 text-center">
              {t("stickerPreview.currentImage")}
            </div>
          ...
          <div class="bg-gray-700 rounded-lg p-3">
            <div class="text-xs text-gray-300 mb-2 text-center">
              {t("stickerPreview.rerollResult")}
            </div>
            <div class="flex justify-center">
              <div class="max-w-full">
                <img
                  src={rerollResult.dataUrl}
                  alt={rerollResult.name}
                  class="max-w-full rounded-lg"
                />
              </div>
            </div>
    ...
    <div>
      <h4 class="text-sm font-medium text-white mb-2">
        {t("stickerPreview.rerollSettings")}
      </h4>
      <div class="bg-gray-700 rounded-lg p-3 space-y-3">
        <div>
          <label for="reroll-prompt" class="block text-xs text-gray-300 mb-1"
            >{t("stickerPreview.prompt")}:</label
          >
          <textarea
            id="reroll-prompt"
            bind:value={rerollPrompt}
        ...
        <div class="grid grid-cols-3 gap-2">
          <div>
            <label for="reroll-steps" class="block text-xs text-gray-300 mb-1"
              >{t("stickerPreview.steps")}:</label
            >
            <input
              id="reroll-steps"
              bind:value={rerollSteps}
              type="number"
              min="1"
              max="50"
          ...
          <div>
            <label for="reroll-scale" class="block text-xs text-gray-300 mb-1"
              >{t("stickerPreview.scale")}:</label
            >
            <input
              id="reroll-scale"
              bind:value={rerollScale}
              type="number"
              min="1"
              max="20"
              step="0.5"
          ...
          <div>
            <label for="reroll-size" class="block text-xs text-gray-300 mb-1"
              >{t("stickerPreview.imageSize")}:</label
            >
            <select
              id="reroll-size"
              bind:value={rerollSize}
              class="w-full px-2 py-1 bg-gray-600 text-white rounded text-xs"
            >
