...
<div class="space-y-4">
    ...
    {#if models.length > 0}
      <div class="grid grid-cols-1 gap-2 mb-3">
        {#each models as model}
          <button
            type="button"
            class="model-select-btn px-3 py-2 text-left text-sm rounded-lg transition-colors {config.model ===
            model
      ...
      <div class="mt-3 space-y-1">
        ...
        {#each customModels as model, index}
          <div class="flex items-center gap-2">
            <button
              type="button"
              class="model-select-btn flex-1 px-3 py-2 text-left text-sm rounded-lg transition-colors {config.model ===
              model
                ? 'bg-blue-600 text-white'
                : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}"
              on:click={() => handleConfigChange("model", model)}
  ...
  <details class="group mt-4">
    <summary
      class="flex items-center justify-between cursor-pointer list-none p-4 bg-gray-700/30 rounded-xl hover:bg-gray-700/40 transition-colors"
    >
      <h4 class="text-sm font-medium text-gray-300 flex items-center">
        <SettingsIcon class="w-4 h-4 mr-2" />{t("settings.advancedSettings")}
      </h4>
      <ChevronDown
        class="w-4 h-4 text-gray-400 transition-transform duration-300 group-open:rotate-180"
      />
    </summary>
    <div class="space-y-4 mt-2 p-4 bg-gray-700/30 rounded-xl">
      ...
      <div>
        ...
        >
          ...
          <span class="text-blue-400 font-mono text-xs"
            >{config.maxTokens || (provider === "gemini" ? 4096 : 4096)}</span
          >
        </label>
        <input
      ...
      <div class="border-t border-gray-600 pt-4 mt-4">
        <h5 class="text-xs font-medium text-gray-400 mb-3">
          {t("settings.profileGenerationSettings")}
        </h5>
        <div class="space-y-3">
          <!-- Profile Max Tokens -->
          <div>
            <label
              for="profile-max-tokens-{provider}"
              class="flex items-center justify-between text-sm font-medium text-gray-300 mb-2"
            >
              <span>{t("settings.profileMaxTokens")}</span>
              <span class="text-blue-400 font-mono text-xs"
                >{config.profileMaxTokens || 1024}</span
              >
            </label>
            <input
          ...
          <div>
            <label
              for="profile-temperature-{provider}"
              class="flex items-center justify-between text-sm font-medium text-gray-300 mb-2"
            >
