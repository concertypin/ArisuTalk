...
<div class="border-t border-gray-800 pt-6">
    <div class="group flex items-center justify-between px-1 mb-2 relative">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-medium text-gray-300">{t("groupChat.groupChat")}</h3>
        </div>
        <button on:click={openCreateGroupChatModal} class="opacity-0 group-hover:opacity-100 transition-opacity p-1 bg-gray-700 hover:bg-blue-600 rounded text-gray-300 hover:text-white transition-colors" title={t("groupChat.newGroupChat")}>
            <Plus class="w-3 h-3" />
        </button>
    </div>
    {#each Object.values($groupChats) as chat (chat.id)}
        ...
        >
            <div class="absolute top-2 right-2 {isSelected ? 'opacity-60 hover:opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity duration-200 flex space-x-1 z-20">
                ...
                <button on:click|stopPropagation={() => deleteGroupChat(chat)} class="p-2 bg-gray-700 hover:bg-red-600 rounded text-gray-300 hover:text-white transition-colors" title={t("groupChat.delete")}>
            ...
            <div class="flex items-center space-x-4 md:space-x-5">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-gray-400 truncate">{chat.participantIds.length}{t("groupChat.participantsCount")}</p>
                    <p class="text-xs text-gray-500 truncate mt-1">{lastMessage?.content || t("groupChat.startConversation")}</p>
                </div>
            </div>
        </div>
    {/each}
