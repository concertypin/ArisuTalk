...
import { messages, selectedChatId, isWaitingForResponse, chatRooms, groupChats, openChats, typingCharacterId, currentMessage, editingMessageId, virtualStream } from '../stores/chat';
...
import { findMessageGroup } from '../../utils.js';
...
async function handleVirtualStream(chatId, character, messageParts) {
  ...
  for (let i = 0; i < messageParts.length; i++) {
    ...
    await new Promise(resolve => setTimeout(resolve, messagePart.delay || 1000));
...
function processAutoPost(character, autoPost) {
    ...
    const currentState = allCharacterStates[character.id] || { affection: 0.3, intimacy: 0.2, trust: 0.25, romantic_interest: 0.0 };
...
async function callApiAndHandleResponse(chatId, character, history, isProactive = false, forceSummary = false) {
    ...
    if (!currentConfig && apiProvider === "gemini") {
        currentConfig = { apiKey: currentSettings.apiKey, model: currentSettings.model };
    }
    ...
    const options = { maxTokens: currentConfig.maxTokens, temperature: currentConfig.temperature };
    ...
    try {
        ...
        const chatType = isGroupChat(chatId) ? 'group' : isOpenChat(chatId) ? 'open' : 'general';
        addLog({
            type: 'structured',
            characterName: character.name,
            chatId: chatId,
            chatType: chatType,
            data: { personaInput: { characterName: character.name, characterPrompt: character.prompt, characterMemories: character.memories, characterId: character.id, }, outputResponse: { messages: response.messages, newMemory: response.newMemory, characterState: response.characterState, reactionDelay: response.reactionDelay, }, parameters: { model: currentConfig.model, isProactive: isProactive, forceSummary: forceSummary, messageCount: history.length, }, metadata: { chatId, chatType: chatType, timestamp: new Date().toISOString(), apiProvider, model: currentConfig.model, }, }
        });

        if (response.characterState) {
            characterStateStore.update(states => ({
                ...states,
                [character.id]: { ...(states[character.id] || {}), ...response.characterState }
            }));
        }
        ...
        if (response.newMemory && response.newMemory.trim() !== "") {
            ...
            characters.update(chars => {
                ...
                if (charIndex !== -1) {
                    ...
                    newChars[charIndex] = processAutoPost(newChars[charIndex], legacyMemoryPost);
                    return newChars;
                }
                return chars;
            });
        ...
        if (response.autoPost) {
            characters.update(chars => {
                ...
                if (charIndex !== -1) {
                    const newChars = [...chars];
                    newChars[charIndex] = processAutoPost(newChars[charIndex], response.autoPost);
                    return newChars;
                }
                return chars;
            });
...
export async function sendMessage(content, type = 'text', payload = {}) {
    console.log(`sendMessage called with chatId: ${get(selectedChatId)}, isGroupChat: ${isGroupChat(get(selectedChatId))}`);
    ...
    if (isGroupChat(chatId)) {
        ...
        const activeParticipants = participants.filter(participantId => {
            const settings = groupSettings.participantSettings[participantId] || { isActive: true, responseProbability: 0.9 };
        ...
        const maxResponders = Math.min(groupSettings.maxRespondingCharacters, activeParticipants.length);
        ...
        for (let i = 0; i < respondingCharacterIds.length; i++) {
            ...
            if (i > 0) {
                await new Promise(resolve => setTimeout(resolve, groupSettings.responseDelay));
            }

            try {
                await callApiAndHandleResponse(chatId, character, history);
            } catch (error) {
                console.error(`Error in group chat response for ${character.name}:`, error);
            }
        }
        ...
        const respondingCharacterId = participants[Math.floor(Math.random() * participants.length)];
        const character = get(characters).find(c => c.id === respondingCharacterId);
...
export async function saveEditedMessage(messageId, newContent) {
    ...
    const character = get(characters).find(c => c.id === getCurrentChatRoom(chatId)?.characterId);
    if (!character) {
        isWaitingForResponse.set(false);
        return;
    }
    ...
    try {
        await callApiAndHandleResponse(chatId, character, updatedMessages, false, true);
    } finally {
        isWaitingForResponse.set(false);
        typingCharacterId.set(null);
    }
...
export async function rerollMessage(messageId) {
    ...
    // For regular chats, use the chat room's characterId
        ...
        character = get(characters).find(c => c.id === getCurrentChatRoom(chatId)?.characterId);
    }

    // Fallback: if character not found by ID, try to find by name
    if (!character && targetMessage.sender && targetMessage.sender !== 'user' && targetMessage.sender !== 'System') {
    ...
    try {
        await callApiAndHandleResponse(chatId, character, truncatedMessages, false, true);
    } finally {
        isWaitingForResponse.set(false);
        typingCharacterId.set(null);
    }
...
export async function generateSnsPost(messageId) {
    ...
    let character = get(characters).find(c => c.id === getCurrentChatRoom(chatId)?.characterId);
    ...
    try {
        const currentSettings = get(settings);
        const recentConversation = currentMessages.slice(-3).map(msg => `${msg.sender}: ${msg.content}`).join('\n');
        ...
        const snsPrompt = snsPromptTemplate
            ...
            .replace(/\{persona\.description\}/g, currentSettings.userDescription || '')
        ...
        const payload = {
            ...
            generationConfig: { temperature: currentConfig.temperature || 1.25, maxOutputTokens: currentConfig.maxTokens || 4096, }
        };
        ...
        let parsedResponse = JSON.parse(responseText.trim().replace(/^```json/, '').replace(/```$/, ''));
        ...
        if (parsedResponse && parsedResponse.autoPost) {
            characters.update(chars => {
                ...
                if (charIndex !== -1) {
                    const newChars = [...chars];
                    newChars[charIndex] = processAutoPost(newChars[charIndex], parsedResponse.autoPost);
                    return newChars;
                }
                return chars;
            });
            ...
            confirmationModalData.set({ title: 'SNS Post Created', message: `SNS post for ${character.name} created: "${parsedResponse.autoPost.content}"`, onConfirm: null });
            isConfirmationModalVisible.set(true);
        } else {
        ...
        confirmationModalData.set({ title: 'Error', message: `Failed to generate SNS post: ${error.message}`, onConfirm: null });
        isConfirmationModalVisible.set(true);
    }
}
