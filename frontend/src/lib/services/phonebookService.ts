...
function parseJsonCharacter(payload: unknown): ImportedCharacterData {
	...
	if (!name || !prompt) {
		throw new Error("Phonebook character payload is missing mandatory fields");
	}
...
async function downloadCharacterData(
	...
	if (!response.ok) {
		throw new Error(`Failed to download phonebook card: ${response.status}`);
	}
	...
	if (IMAGE_MIME_PATTERN.test(contentType)) {
		...
		return parsePngCharacter(new Uint8Array(buffer), contentType.split(";")[0]);
	}
