...
export class SecureStorageManager {
    ...
    async initialize() {
        try {
            ...
            const hasExistingEncryption = await loadFromBrowserStorage(STORAGE_KEYS.ENCRYPTION_ENABLED, false);
            
            if (!hasExistingEncryption) {
                // First time setup - enable encryption automatically
                const autoPassword = this.generateMasterPassword();
                await this.setupEncryption(autoPassword, 'Auto-generated for security');
            } else {
                // Existing setup - restore from session or regenerate
                this.encryptionEnabled = true;
                this.masterPassword = sessionStorage.getItem(STORAGE_KEYS.SESSION_MASTER_PASSWORD);
                
                if (!this.masterPassword) {
                    // Session expired, regenerate password (user won't notice)
                    const autoPassword = this.generateMasterPassword();
                    this.masterPassword = autoPassword;
                    sessionStorage.setItem(STORAGE_KEYS.SESSION_MASTER_PASSWORD, autoPassword);
                }
            }
    ...
    async setupEncryption(masterPassword, hint = '') {
        try {
            ...
            sessionStorage.setItem(STORAGE_KEYS.SESSION_MASTER_PASSWORD, masterPassword);
            ...
            throw new Error('암호화 설정에 실패했습니다.');
        }
    }
    ...
    async saveApiConfigs(apiConfigs) {
        if (!this.masterPassword) {
            console.warn('No master password available for encryption');
            return;
        }
        ...
        try {
            ...
            throw new Error('API 설정 저장에 실패했습니다.');
        }
    }
    ...
    async loadApiConfigs() {
        if (!this.masterPassword) {
            console.warn('No master password available for decryption');
            return {};
        }

        try {
            const encrypted = await loadFromBrowserStorage(STORAGE_KEYS.ENCRYPTED_API_CONFIGS, null);
            ...
            console.error('Failed to load encrypted API configs:', error);
            return {};
        }
    }
